    ...
    pub fn from_row(row: &DbRow) -> Result<Self, PersistenceError> {
        Ok(Self {
            id: row.get_text("ID_").ok_or_else(|| {
                PersistenceError::Deserialization("Missing ID_ in JobEntity".to_string())
            })?,
            ...
            history_url: row.get_text("HISTORY_URL_"),
...
impl RevisionedEntity for JobEntity {
...
impl JobDataManager {
    ...
    pub fn insert(
        ...
        params.push(entity.history_url.clone());
        params.push(entity.handler_type.clone());
        params.push(entity.custom_values_id.clone());

        session.insert(entity, StatementId::InsertAsyncJob, params)
    }
    ...
    pub fn update(
        ...
        session.update(entity, StatementId::UpdateAsyncJob, params)
    }
    ...
    pub fn delete(
        ...
        session.delete(entity, StatementId::DeleteAsyncJob, params)
    }
    ...
    pub fn find_by_id(
        ...
        session: &mut DbSession,
        id: &str,
    ) -> Result<Option<JobEntity>, PersistenceError> {
        let mut params = DbParams::new();
        params.push(id);

        let row = session.select_one(StatementId::SelectAsyncJobById, params)?;
        match row {
            Some(row) => Ok(Some(JobEntity::from_row(&row)?)),
            None => Ok(None),
        }
    }
}

impl Default for JobDataManager {
    fn default() -> Self {
        Self::new()
    }
}
