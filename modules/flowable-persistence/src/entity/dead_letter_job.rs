...
#[derive(Debug, Clone)]
pub struct DeadLetterJobEntity {
    pub id: String,
    pub revision: i32,
    ...
    pub fn from_row(row: &DbRow) -> Result<Self, PersistenceError> {
        Ok(Self {
            id: row.get_text("ID_").ok_or_else(|| {
                PersistenceError::Deserialization("Missing ID_ in DeadLetterJobEntity".to_string())
            })?,
...
impl Entity for DeadLetterJobEntity {
    fn id(&self) -> &str {
        &self.id
    }
...
impl RevisionedEntity for DeadLetterJobEntity {
...
pub struct DeadLetterJobDataManager;

impl DeadLetterJobDataManager {
    pub fn new() -> Self {
        Self
    }
    ...
    pub fn insert(
        ...
        session.insert(entity, StatementId::InsertDeadLetterJob, params)
    }
    ...
    pub fn update(
        ...
        session.update(entity, StatementId::UpdateDeadLetterJob, params)
    }
    ...
    pub fn delete(
        ...
        session.delete(entity, StatementId::DeleteDeadLetterJob, params)
    }
    ...
    pub fn find_by_id(
        ...
        session: &mut DbSession,
        id: &str,
    ) -> Result<Option<DeadLetterJobEntity>, PersistenceError> {
        let mut params = DbParams::new();
        params.push(id);

        let row = session.select_one(StatementId::SelectDeadLetterJobById, params)?;
        match row {
            Some(row) => Ok(Some(DeadLetterJobEntity::from_row(&row)?)),
            None => Ok(None),
        }
    }
}

impl Default for DeadLetterJobDataManager {
    fn default() -> Self {
        Self::new()
    }
}
