    ...
    pub fn from_row(row: &DbRow) -> Result<Self, PersistenceError> {
        Ok(Self {
            id: row.get_text("ID_").ok_or_else(|| {
                PersistenceError::Deserialization("Missing ID_ in TimerJobEntity".to_string())
            })?,
...
impl Entity for TimerJobEntity {
    fn id(&self) -> &str {
        &self.id
    }
...
impl RevisionedEntity for TimerJobEntity {
...
pub struct TimerJobDataManager;

impl TimerJobDataManager {
    pub fn new() -> Self {
        Self
    }
    ...
    pub fn insert(
        ...
        session.insert(entity, StatementId::InsertTimerJob, params)
    }
    ...
    pub fn update(
        ...
        session.update(entity, StatementId::UpdateTimerJob, params)
    }
    ...
    pub fn delete(
        ...
        session.delete(entity, StatementId::DeleteTimerJob, params)
    }
    ...
    pub fn find_by_id(
        ...
        session: &mut DbSession,
        id: &str,
    ) -> Result<Option<TimerJobEntity>, PersistenceError> {
        let mut params = DbParams::new();
        params.push(id);

        let row = session.select_one(StatementId::SelectTimerJobById, params)?;
        match row {
            Some(row) => Ok(Some(TimerJobEntity::from_row(&row)?)),
            None => Ok(None),
        }
    }
    ...
    pub fn find_due(
        ...
        session: &mut DbSession,
        now: i64,
    ) -> Result<Vec<TimerJobEntity>, PersistenceError> {
        let mut params = DbParams::new();
        params.push(now);

        let rows = session.select_list(StatementId::SelectDueTimerJobs, params)?;
        rows.iter().map(TimerJobEntity::from_row).collect()
    }

    // acquire_due (FOR UPDATE SKIP LOCKED) removed in C4 — see dialect.rs
    // decision note. Engine job acquisition uses optimistic CAS / global lock
    // on runtime JSON state, not this relational SKIP LOCKED path.
}

impl Default for TimerJobDataManager {
    fn default() -> Self {
        Self::new()
    }
}
