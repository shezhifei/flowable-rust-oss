...
#[test]
fn test_timer_coordination_jwks_profile_mutation() {
    let db_path = format!(
        "file:test_jwks_profile_mutation_{}?mode=memory&cache=shared",
        Uuid::new_v4()
    );
    ...
    // Setup an admin secret so we can update the profile
    ...
    // Profile is already seeded by startup, fetch it
    let profile = admin_client.get_issuer_profile("profile-1").unwrap();

    let token = create_token("https://auth.example.com", "test-kid");

    // 1. Auth succeeds, caches the key.
    ...
    // 2. Change the profile's jwks_uri to "invalid-uri", which will cause fetching to fail.
    // Since the key is cached, if we DID NOT invalidate, auth would still succeed!
    // But because we invalidate on update, it should fail to fetch from "invalid-uri" and reject auth.
    ...
    // 3. Auth should fail because the cache is invalidated and it cannot fetch from the new URI.
    let status_res = user_client.get_status();
    assert!(
        status_res.is_err(),
        "Expected auth to fail after jwks_uri update invalidated cache"
    );

    // 4. Change back to "test-local"
    let mut updated_profile = admin_client.get_issuer_profile("profile-1").unwrap();
    updated_profile.jwks_uri = Some("test-local".to_string());
    admin_client
        .update_issuer_profile(&updated_profile)
        .unwrap();
    assert!(
        user_client.get_status().is_ok(),
        "Expected auth to succeed after fixing jwks_uri"
    );

    // 5. Delete profile entirely
    admin_client.delete_issuer_profile("profile-1").unwrap();

    // Auth should fail because profile is gone (and cache is invalidated, though DB check fails first)
